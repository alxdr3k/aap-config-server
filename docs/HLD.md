...
|-------|-------------|----------|---------|-------|
| **FR-1** | Git-backed In-memory Config Store | 4.1 | 1.1, 6.3, 6.4, 6.5 | 1 |
...
| **FR-9** | Config Agent (중앙 집중형) | 4.9 | 2.4, 2.6 | 3 |
| **FR-10** | 설정 상속 (Config Inheritance) | 4.10 | 6 | 5 |
| **FR-11** | 서비스 탐색 API | 4.11 | 2 (API 목록) | 5 |
| **FR-12** | 시크릿 메타데이터 API | 4.12 | 2 (API 목록) | 5 |
| **FR-13** | 변경 이력 API | 4.13 | 2 (API 목록) | 5 |
| **FR-14** | 설정 롤백 API | 4.14 | 2.5 | 5 |
| **FR-15** | 헬스체크 / 운영 API | 4.15 | 2 (API 목록) | 1 |
| **FR-16** | 인증/인가 (App ID + RBAC) | 4.16 | 2.3, 9 | 4 |
...
| **FR-2** | 설정 조회 API | `store` | `GET /config` | — | — |
| **FR-3** | 환경변수 조회 API | `store` | `GET /env_vars` | — | — |
| **FR-4** | 설정/시크릿 일괄 변경 | `store`, `git`, `seal` | `POST /admin/changes` | Git, kubeseal, K8s API | — |
| **FR-5** | 설정/시크릿 일괄 삭제 | `store`, `git` | `DELETE /admin/changes` | Git | — |
| **FR-6** | 변경 감지 API (watch) | `store` | `GET /config/watch`, `GET /env_vars/watch` | — | watch client |
...
| **FR-11** | 서비스 탐색 API | `store` | `GET /orgs`, `/projects`, `/services` | — | — |
| **FR-12** | 시크릿 메타데이터 API | `store` | `GET /secrets` | — | — |
