...
본 HLD는 [PRD v1.5](./PRD.md)에 정의된 기능 요구사항(FR-1 ~ FR-17)을 설계 수준에서 구현한다. 각 HLD 섹션에 `[FR-X]` 태그를 표기하여 PRD 요구사항과의 추적성을 보장한다.

### 0.1 FR → HLD 매핑

| FR ID | 기능 요구사항 | PRD 섹션 | HLD 섹션 | Phase |
|-------|-------------|----------|---------|-------|
| **FR-1** | Git-backed In-memory Config Store | 2.1, 2.2, 7 | 1.1, 6.3, 6.4, 6.5 | 1 |
| **FR-2** | 설정 조회 API | 6.1 | 2 (API 목록) | 1 |
| **FR-3** | 환경변수 조회 API | 6.1 | 2 (API 목록) | 1 |
| **FR-4** | 설정/시크릿 일괄 변경 Admin API | 6.4 | 2.1, 4.1, 4.2, 4.3 | 1 |
| **FR-5** | 설정/시크릿 일괄 삭제 Admin API | 6.4 | 2.1 | 1 |
| **FR-6** | 설정/환경변수 변경 감지 (Long Polling) | 6.2, 6.9 | 2.4 | 5 |
| **FR-7** | 시크릿 관리 (SealedSecret + Volume Mount) | 3.4, 5 | 2.2, 3, 4.3 | 2 |
| **FR-8** | App Registry 연동 | 3.3 | 2.3 | 2 |
| **FR-9** | Config Agent (중앙 집중형) | 4 | 2.4, 2.6 | 3 |
| **FR-10** | 설정 상속 (Config Inheritance) | 3.5 | 6 | 5 |
| **FR-11** | 서비스 탐색 API | 6.6 | 2 (API 목록) | 5 |
| **FR-12** | 시크릿 메타데이터 API | 6.5 | 2 (API 목록) | 5 |
| **FR-13** | 변경 이력 API | 6.7 | 2 (API 목록) | 5 |
| **FR-14** | 설정 롤백 API | 6.8 | 2.5 | 5 |
| **FR-15** | 헬스체크 / 운영 API | 6.3 | 2 (API 목록) | 1 |
| **FR-16** | 인증/인가 (App ID + RBAC) | 3.3, 8.1 | 2.3, 9 | 4 |
| **FR-17** | 시크릿 보안 | 5, 8.2, 8.3 | 3.4, 9 | 4 |
...
### 2.1 설정 쓰기 흐름 (Console → Config Server → Git) `[FR-4]` `[FR-5]`
...
### 2.2 시크릿 처리 흐름 (changes API 내부) `[FR-7]`
...
### 2.3 App Registry (Project/App CRUD) 흐름 `[FR-8]` `[FR-16]`
