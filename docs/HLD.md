...
rules:
  ...
  # Secret 값 조회 (resolve_secrets API용)
  - apiGroups: [""]
    resources: ["secrets"]
    verbs: ["get", "list"]
...
**반영 방식**: ConfigMap 업데이트 → kubelet 자동 전파 → litellm hot reload (Pod 재시작 없음)
...
| 시크릿 평문 (K8s Secret) | K8s etcd | base64 (etcd encryption at rest) | Config Server (API 조회), kubelet (Volume Mount) |
...
|------|----------------|------------|------|
| **중앙 Agent (replica=1)** | 1 | Rolling update 보장 | **채택** |
...
| K8s API | Secret 조회 (resolve_secrets), SealedSecret apply |
