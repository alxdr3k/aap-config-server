...
3. **설정 쓰기 시**: Console Admin API (`POST /admin/changes`) 수신 → 스키마 검증 → 설정/시크릿 일괄 처리 → 단일 Git commit & push → 메모리 갱신
4. **설정 변경 감지 시**: Git webhook 또는 주기적 poll → 변경 감지 → 메모리 갱신
5. **시크릿 요청 시**: 메타데이터(Git) + 시크릿 값(K8s Volume Mount) 조합 → 클러스터 내부 네트워크로 응답
6. **시크릿 생성/변경 시**: `POST /admin/changes`의 `secrets` 필드 수신 → kubeseal 암호화 → SealedSecret YAML Git push + K8s apply → SealedSecret Controller가 K8s Secret 생성
...
> **레포 통합 이유**: `POST /admin/changes`에서 config + secrets를 단일 atomic Git commit으로 처리하려면 모든 파일이 같은 레포에 있어야 한다. `aap-helm-charts`는 이미 AAP 서비스 배포의 중심 레포이며, SealedSecret YAML은 본질적으로 K8s 배포 매니페스트이므로 여기에 저장하는 것이 자연스럽다.
>
...
Console이 `POST /admin/changes`의 `secrets` 필드로 시크릿 평문을 전송하면, Config Server가 kubeseal 암호화 → SealedSecret YAML Git commit & push → K8s API apply를 처리한다. 이후 SealedSecret Controller 복호화 → K8s Secret 생성 → kubelet Volume Mount sync (~60초)로 Config Server Pod에 반영된다.
...
| API 인증 | **Config Server 발급 API Key** — `Authorization: Bearer <api-key>` 헤더로 전송 |
| 접근 제어 | API Key에 연결된 scope 및 permissions로 요청 범위 제한 |
| 시크릿 접근 제어 | API Key의 `resolve_secrets` 권한 확인 |

#### API Key 관리

Config Server가 API Key의 발급·검증 주체이다. Console은 발급받은 API Key를 안전하게 보관하고 Admin API 호출 시 Bearer 토큰으로 전송한다.

| 항목 | 상세 |
|------|------|
| **발급** | Config Server Admin API (`POST /api/v1/admin/api-keys`)로 API Key 생성. 응답에 평문 키를 **일회성**으로 반환 |
| **저장 (Config Server)** | API Key는 해시(SHA-256)로만 저장. 평문은 발급 시점 이후 Config Server에 존재하지 않음 |
| **저장 (Console)** | Console이 발급받은 평문 API Key를 자체 DB 또는 K8s Secret에 안전하게 보관 |
| **검증** | 요청의 `Authorization: Bearer <key>`에서 키를 추출 → SHA-256 해시 → 저장된 해시와 비교 |
| **scope** | API Key 생성 시 접근 가능한 org/project/service 범위와 권한(read, write, resolve_secrets 등)을 지정 |
| **폐기** | `DELETE /api/v1/admin/api-keys/{key_id}`로 즉시 폐기. 폐기된 키로의 요청은 `401 Unauthorized` |
| **순환 (Rotation)** | 새 키 발급 → Console 측 교체 → 이전 키 폐기. 두 키가 동시에 유효한 grace period 지원 |

#### API Key 저장소

Config Server는 **No Database** 원칙을 유지하면서 API Key를 관리한다:

| 항목 | 상세 |
|------|------|
| **저장 위치** | Git 저장소 (`configs/_api-keys/keys.yaml`) — 해시된 키 + 메타데이터 |
| **인메모리 캐시** | 시작 시 Git에서 로드, 발급/폐기 시 즉시 갱신 |
| **Git 커밋** | 키 발급/폐기 시 자동 Git commit (이력 추적) |

```yaml
# configs/_api-keys/keys.yaml 예시
api_keys:
  - id: "key-a1b2c3d4"
    hash: "sha256:e3b0c44298fc1c149afb..."   # SHA-256 해시
    name: "console-admin"                     # 사람이 읽을 수 있는 이름
    scope:
      org: "*"                                # 전체 org 접근
      permissions: ["read", "write", "resolve_secrets"]
...
> **초기 부트스트랩**: Config Server 최초 설치 시, 환경변수 `BOOTSTRAP_API_KEY`로 초기 관리자 API Key를 설정할 수 있다. 이 키로 첫 번째 정식 API Key를 발급한 후 부트스트랩 키는 비활성화한다.
...
| **감사 로깅** | 시크릿 접근 시 API Key ID, 시간, 요청 scope을 감사 로그에 기록 |
...
| POST | `/api/v1/admin/api-keys` | API Key 발급 (해시 저장, 평문 일회성 반환) |
| DELETE | `/api/v1/admin/api-keys/{key_id}` | API Key 폐기 |
...
- [ ] `[FR-1]` In-memory config store 구현 (COW 패턴)
...
- [ ] `[FR-16]` API Key Bearer 인증 미들웨어 (`Authorization: Bearer <key>` 검증)
- [ ] `[FR-16]` API Key 발급/폐기 Admin API (`POST/DELETE /api/v1/admin/api-keys`)
- [ ] `[FR-16]` API Key Git 저장소 관리 (`configs/_api-keys/keys.yaml`)
- [ ] `[FR-16]` API Key scope 기반 인가 검증
- [ ] `[FR-16]` Bootstrap API Key 환경변수 지원 (`BOOTSTRAP_API_KEY`)
