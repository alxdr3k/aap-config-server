...
│  │                                                         │  │
│  │  /app/config.yaml ← ConfigMap volume mount (subPath)    │  │
│  │  /env/env.sh      ← Secret volume mount                │  │
│  └─────────────────────────────────────────────────────────┘  │
│                                                               │
│  Volumes:                                                     │
│  - /app/config.yaml (ConfigMap: litellm-config, subPath)     │
...
volumeMounts:
  - name: config
    ...
    subPath: config.yaml                # litellm 기본 config 경로에 직접 마운트
    readOnly: true
  - name: env
    mountPath: /env
    readOnly: true
...
```bash
source /env/env.sh && litellm --config /app/config.yaml
```
