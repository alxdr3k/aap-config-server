...
| **Secret Encryption** | 시크릿 전송은 mTLS로 보호, 저장은 K8s Secret(etcd encryption at rest)에 위임 |
...
└─────────────────────────┼────────────────────────────────────┘
              ...
              │   (fetch, write       │  per client service Pod
              │    config files)      │
              ├───────────────────────┤
...
4. **시크릿 요청 시**: 메타데이터(Git) + 시크릿 값(K8s Volume Mount) 조합 → mTLS 채널로 응답 (평문 시크릿은 TLS로 보호)
...
mTLS 사용 시 LB의 동작 모드에 따라 보안 특성이 달라진다:

| LB 모드 | 동작 | 시크릿 노출 | 비고 |
|---------|------|------------|------|
| **L4 passthrough** (권장) | TCP 레벨 포워딩, TLS 미종료 | LB에서 평문 접근 **불가** | mTLS 종단 간 보호 유지 |
| **L7 termination** | LB에서 TLS 종료 후 재암호화 | LB에서 평문 접근 **가능** | LB 자체의 보안이 중요 |

시크릿 전송 보안을 위해 **L4 passthrough**를 권장한다. L7 termination이 필요한 경우(HTTP 라우팅 등), LB의 로깅 비활성화 및 접근 제어를 별도로 설정해야 한다.
...
- **Admin**이 AAP Console에서 App을 등록하고 App ID + App Secret을 발급
...
1. **Init 단계**: Config Server에서 설정 + 환경변수 fetch (시크릿 포함) → 로컬 파일로 기록
...
│  │  2. 응답에서 시크릿이 resolve된 설정 수신 (mTLS로 보호)    │  │
...
Config Server의 응답에서 `config` 블록을 추출하고, litellm이 읽을 수 있는 `proxy_config` 형식으로 기록:
...
```bash
# /shared/env.sh (Config Agent가 생성)
...
시크릿 값의 보호는 **별도의 페이로드 암호화 없이**, 검증된 인프라 계층에 위임한다.

### 5.1 보안 전략: mTLS + K8s Secret
...
┌─ 전송 (in transit) ──────────────────────────────────────────────┐
│                                                                  │
│  Config Server ←──(mTLS)──→ Config Agent                         │
│  → TLS 1.3으로 채널 자체를 암호화                                   │
│  → 중간자 공격 방지 (상호 인증서 검증)                               │
│  → 시크릿은 TLS 터널 안에서만 평문으로 전달                          │
│                                                                  │
└──────────────────────────────────────────────────────────────────┘
```

**왜 별도 페이로드 암호화(ECDH 등)를 하지 않는가**:

| 고려사항 | 판단 |
|----------|------|
| **mTLS 충분성** | TLS 1.3은 채널 암호화 + 상호 인증을 동시에 제공. 중간 프록시/LB도 인증서 없이 복호화 불가 |
| **운영 복잡도** | 앱별 ECDH 키 생성/등록/순환은 운영 부담이 큼. mTLS 인증서는 cert-manager 등으로 자동화 가능 |
| **K8s 네이티브** | K8s Secret + etcd encryption at rest는 이미 검증된 보안 패턴 |
| **로깅/캐싱 방어** | `Cache-Control: no-store` 헤더 + 시크릿 포함 응답 로깅 금지 정책으로 충분 |

> **향후 필요 시**: 규제 요건 등으로 페이로드 레벨 암호화가 필요해지면, 앱별이 아닌 **Config Server ↔ Config Agent 간 단일 키 쌍**으로 E2E 암호화를 추가할 수 있다. 앱별 키 관리의 복잡도 없이 동일한 보안 효과를 얻을 수 있다.

### 5.2 응답 형식

`resolve_secrets=true`로 요청 시 시크릿이 **평문으로 resolve되어** 응답에 포함된다 (mTLS로 전송 보호):
...
4. Config Server: 응답 전송
   - 시크릿이 resolve된 설정 응답 (mTLS로 전송 보호)
...
| 파라미터 | 타입 | 설명 |
|----------|------|------|
| `resolve_secrets` | bool | `true`면 시크릿 값을 resolve하여 평문으로 포함, mTLS로 전송 보호 (default: `false`) |
...
**Response** (`resolve_secrets=true`): `secret_refs`의 각 값이 실제 시크릿 평문으로 치환되어 응답 (mTLS로 전송 보호)
...
| 계층 | 방식 |
|------|------|
| 서비스 간 통신 | mTLS (전송 채널 암호화, 시크릿 전송 보호 포함) |
...
| **전송 시 암호화** | mTLS로 채널 보호 (TLS 1.3, 상호 인증서 검증) |
...
| 위협 | 방어 |
|------|------|
| 네트워크 스니핑 | mTLS로 채널 암호화, 인증서 없이 복호화 불가 |
| 프록시/LB 로깅 | L4 passthrough 권장 → 중간 장비가 평문 접근 불가. L7 termination 시 LB 로깅 비활성화 필수. 추가로 `Cache-Control: no-store` 적용 (2.3절 참조) |
| 서버 메모리 덤프 | 시크릿은 요청 처리 중에만 메모리에 존재, 처리 후 제로화 |
| Git 저장소 유출 | 시크릿 값이 Git에 없으므로 영향 없음 (메타데이터만 노출) |
| mTLS 인증서 유출 | 인증서 순환(cert-manager 자동화)으로 영향 최소화 |
...
| TLS | `crypto/tls` (stdlib) | mTLS 지원, TLS 1.3 기본 |
...
- [ ] gzip 응답 압축
- [ ] mTLS 인증서 자동 순환 (cert-manager 연동)
