...
> **작성일**: 2026-03-12
> **최종 수정일**: 2026-04-23
> **상태**: **Approved design — Phase-1 implementation in progress.** 문서가 설명하는 전체 v2.1 계약(SealedSecret 생성, Config Agent, watch/history/revert, resolve_secrets 등) 중 상당 부분은 아직 구현되지 않았다. 실제로 현재 제공되는 범위는 [README.md](../README.md#feature-matrix) 의 Feature Matrix를 기준으로 한다. 본 문서의 API/저장소/아키텍처 기술은 **목표 계약**이며, 구현되지 않은 엔드포인트·필드는 Phase-1에서 명시적으로 거부된다 (예: `POST /admin/changes` 의 `secrets` 필드는 400).
> **참조**: [HLD](./HLD.md) · [development-process.md](./development-process.md) · [README.md Feature Matrix](../README.md#feature-matrix)
...
> **Go 설계 원칙**: 프로젝트 구조, 의존성 주입, context.Context 전파, 에러 처리 전략, graceful shutdown, 서버 자체 설정 로딩 등 Go 관용적 설계 원칙은 [HLD 섹션 11](./HLD.md#11-go-설계-원칙)에서 상세히 정의한다.
...
- [ ] Go 프로젝트 구조 세팅 (`go mod init`, `cmd/`, `internal/` 디렉토리 — [HLD 섹션 11.1](./HLD.md#111-프로젝트-구조) 참조)
- [ ] 서버 자체 설정 로딩 (`internal/config` — 환경변수 + 플래그)
- [ ] 커스텀 에러 타입 정의 (`internal/apperror` — [HLD 섹션 11.4](./HLD.md#114-에러-처리) 참조)
