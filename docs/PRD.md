...
1. **Console**이 `POST /admin/changes`에 `secrets` 필드를 포함하여 Config Server에 전송
...
### 6.4 설정/시크릿 일괄 변경 API (Console → Config Server Admin API)
...
- [ ] Go 프로젝트 구조 세팅
- [ ] `aap-helm-charts` 저장소 clone/pull 및 `configs/` 하위 설정 파일 파싱 (`config.yaml`, `env_vars.yaml`, `secrets.yaml`)
- [ ] In-memory config store 구현 (COW 패턴)
- [ ] REST API: 설정 조회 (`GET /api/v1/.../config`)
- [ ] REST API: 환경변수 조회 (`GET /api/v1/.../env_vars`)
- [ ] Admin API: 설정/시크릿 일괄 변경 (`POST /api/v1/admin/changes`) — 스키마 검증 + kubeseal + 단일 Git commit & push
- [ ] Admin API: 설정/시크릿 일괄 삭제 (`DELETE /api/v1/admin/changes`) — Git에서 파일 삭제 + commit & push
...
- [ ] `POST /admin/changes`의 `secrets` 필드 처리 → kubeseal 암호화 + SealedSecret apply
...
- [ ] 버전 롤백 API (`POST /api/v1/admin/changes/revert`)
