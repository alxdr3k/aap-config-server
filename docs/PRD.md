...
> **향후 필요 시**: 규제 요건 등으로 페이로드 레벨 암호화가 필요해지면, 앱별이 아닌 **Console ↔ Config Server 간 단일 키 쌍**으로 E2E 암호화를 추가할 수 있다. 앱별 키 관리의 복잡도 없이 동일한 보안 효과를 얻을 수 있다.
...
- 시크릿 전송 보안은 mTLS에 위임하여 별도 암호화 오버헤드 없음
- 시크릿 포함 응답에 `Cache-Control: no-store` 헤더 적용
- 시크릿은 메모리에서만 존재, 사용 후 즉시 제로화
...
| 프록시/LB 로깅 | mTLS 종단 간 암호화 → 중간 장비가 평문 접근 불가. 추가로 `Cache-Control: no-store` 적용 |
