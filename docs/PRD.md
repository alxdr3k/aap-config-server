...
3. **설정 쓰기 시**: Console Admin API (`POST /admin/configs`) 수신 → 스키마 검증 → Git commit & push → 메모리 갱신
...
```
Console → Config Server → kubeseal 암호화 → Git commit → K8s API apply
                                                              │
                                              SealedSecret Controller
                                              복호화 → K8s Secret
                                                              │
...
- `POST /api/v1/admin/configs`: `service` 레벨 설정만 쓴다. `_defaults` 디렉토리는 직접 수정하지 않는다.
...
### 6.4 설정 쓰기 API (Console → Config Server Admin API)

Console이 설정을 생성/변경/삭제할 때 Config Server Admin API를 통해 Git에 반영한다. 시크릿 webhook과 동일한 원칙: **Console은 "무엇을 설정할지"만 결정하고, Config Server가 "어떻게 저장하고 전파할지"를 담당한다.** Console은 Git/kubeseal/kubectl 의존성 없이 Admin API만 호출한다.
...
- `config`: config.yaml의 `config` 블록 전체 (optional — 생략 시 config.yaml 미변경)
- `env_vars`: env_vars.yaml 전체 (optional — 생략 시 env_vars.yaml 미변경)
...
3. config.yaml / env_vars.yaml 파일 생성/업데이트 → **단일 Git commit** & push
4. In-memory config store 갱신
5. 다음 Config Agent polling 시 변경 감지 → ConfigMap/Secret 업데이트 → rolling restart

> **Atomic Commit 원칙**: 하나의 API 호출로 발생하는 모든 파일 변경(config.yaml, env_vars.yaml 등)은 **반드시 단일 Git commit**으로 처리한다. 시크릿 webhook에 의한 SealedSecret 변경도 마찬가지다. 이 commit hash가 해당 변경의 유일한 버전 식별자가 되며, Console은 이 값을 DB에 저장하여 이력 추적 및 롤백에 사용한다.
...
Project 삭제 시 해당 서비스의 설정 전체를 제거한다. Console이 Project 삭제 워크플로우에서 호출한다.
...
**처리 흐름**:
1. 해당 서비스 디렉토리의 config.yaml, env_vars.yaml, secrets.yaml 삭제
2. Git commit & push
3. In-memory store에서 해당 서비스 설정 제거
4. Config Agent가 다음 polling 시 변경 감지 → ConfigMap/Secret 정리

> **주의**: sealed-secrets/ 디렉토리의 SealedSecret 삭제는 별도의 시크릿 webhook (`action: delete`)으로 처리한다. 설정 삭제와 시크릿 삭제는 Console이 각각 호출한다.

#### 설정 검증 (Dry-run)

```
POST /api/v1/admin/configs/validate
```

Git commit 없이 설정의 유효성만 검증한다. Console이 사용자 입력을 저장하기 전에 사전 검증할 때 사용한다.

**Request Body**: `POST /api/v1/admin/configs`와 동일
...
```
POST /api/v1/admin/configs/rollback
```
...
### 6.9 환경변수 변경 감지 API (Long Polling)
...
| DELETE | `/api/v1/admin/configs` | 설정 삭제 (Project 삭제 시 설정 파일 제거 → Git commit & push) |
...
| POST | `/api/v1/admin/secrets/webhook` | 시크릿 생성/변경/삭제 (kubeseal 암호화 → Git push + K8s API apply) |
...
- [ ] Admin API: 설정 생성/변경 (`POST /api/v1/admin/configs`) — 스키마 검증 + Git commit & push
- [ ] Admin API: 설정 삭제 (`DELETE /api/v1/admin/configs`) — Git에서 파일 삭제 + commit & push
- [ ] Admin API: 설정 검증 (`POST /api/v1/admin/configs/validate`) — dry-run
...
- [ ] SealedSecret K8s API apply (client-go)
...
- [ ] 설정 롤백 API (`POST /api/v1/admin/configs/rollback`)
