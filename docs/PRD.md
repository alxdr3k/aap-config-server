...
`resolve_secrets=true` 요청 시 Config Server는: App ID 인증/인가 검증 → `env_vars.yaml`의 `secret_refs` 탐지 → `secrets.yaml`에서 K8s Secret 경로 확인 → Volume Mount 파일에서 시크릿 값 읽기(`/secrets/{namespace}/{name}/{key}`) → 평문 환경변수 + 시크릿 환경변수를 합산하여 응답 (`Cache-Control: no-store`).
...
각 클라이언트 서비스(litellm 등)는 AAP Console에서 발급받은 **App ID**로 자신을 식별한다. Console이 App 등록/수정/삭제 시 Config Server로 webhook push(**fire-and-forget + async retry**)하여 인메모리 캐시를 갱신한다. Config Server는 캐시에서 App ID의 유효성과 scope을 검증한다.
...
- [ ] `[FR-16]` App ID 인증 미들웨어
- [ ] `[FR-16]` App Registry 기반 scope 인가 검증
