...
> **작성일**: 2026-03-11
> **최종 수정일**: 2026-04-29
> **상태**: **Approved design — Phase-1 implementation in progress.** 본 문서의 패키지 구조(`cmd/config-agent`, `internal/seal`, `internal/auth`, `internal/agent` 등) 및 Config Agent 흐름은 **목표 설계**이며, 현재 repo에는 Config Server 본체(`cmd/config-server`, `internal/{config,gitops,handler,parser,registry,server,store,apperror}`)와 secret runtime/volume/sealing/apply/audit boundary(`internal/secret`)가 구현되어 있다. 실제 구현 범위는 [README.md Feature Matrix](../README.md#feature-matrix) 참조.
