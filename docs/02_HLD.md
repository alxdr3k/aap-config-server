...
> **작성일**: 2026-03-11
> **최종 수정일**: 2026-04-30
> **상태**: **Approved design — Phase-1 implementation in progress.** 본 문서의 패키지 구조 중 `cmd/config-agent`와 `internal/agent` bootstrap은 구현되어 있으나, Config Agent의 K8s leader election/apply/rollout 흐름과 `internal/seal`, `internal/auth` 등 일부 목표 패키지는 아직 **목표 설계**이다. 현재 repo에는 Config Server 본체(`cmd/config-server`, `internal/{config,gitops,handler,parser,registry,server,store,apperror}`), Agent bootstrap(`cmd/config-agent`, `internal/agent`), secret runtime/volume/sealing/apply/audit boundary(`internal/secret`)가 구현되어 있다. 실제 구현 범위는 [README.md Feature Matrix](../README.md#feature-matrix) 참조.
