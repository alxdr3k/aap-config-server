...
> **작성일**: 2026-03-11
> **최종 수정일**: 2026-04-30
> **상태**: **Approved design — Phase-1 implementation in progress.** 본 문서의 패키지 구조 중 `cmd/config-agent`와 `internal/agent` bootstrap, K8s Lease leader election, read polling/version tracking module, native config/env.sh rendering module, ConfigMap/Secret apply module, Deployment rollout patch module, leading-edge debounce module은 구현되어 있으나, Config Agent의 image/RBAC/e2e orchestration 흐름과 `internal/seal`, `internal/auth` 등 일부 목표 패키지는 아직 **목표 설계**이다. 현재 repo에는 Config Server 본체(`cmd/config-server`, `internal/{config,gitops,handler,parser,registry,server,store,apperror}`), Agent bootstrap/leader election/fetch loop/rendering/apply/rollout patch/debounce(`cmd/config-agent`, `internal/agent`), secret runtime/volume/sealing/apply/audit boundary(`internal/secret`)가 구현되어 있다. 실제 구현 범위는 [README.md Feature Matrix](../README.md#feature-matrix) 참조.
...
본 HLD는 [PRD v2.1](./01_PRD.md)에 정의된 기능 요구사항(FR-1 ~ FR-17)을 설계 수준에서 구현한다. 각 HLD 섹션에 `[FR-X]` 태그를 표기하여 PRD 요구사항과의 추적성을 보장한다.
...
|-----------|-------------|-----------------|-------------------|
| Console FR-3 | Project 삭제 (롤백) | FR-5, FR-8 | `DELETE /api/v1/admin/changes`, `POST /api/v1/admin/app-registry/webhook` |
| Console FR-5 | Langfuse SK/PK 전달 | FR-4, FR-7 | `POST /api/v1/admin/changes` (secrets 필드) |
| Console FR-6 | LiteLLM Config 반영 | FR-4 | `POST /api/v1/admin/changes` (config 필드) |
| Console FR-7 | 프로비저닝 파이프라인 | FR-4, FR-8 | `POST /api/v1/admin/changes`, `POST /api/v1/admin/app-registry/webhook` |
| Console FR-8 | 설정 버전 롤백 | FR-14 | `POST /api/v1/admin/changes/revert` |
...
Console이 설정/시크릿을 변경하면, Config Server의 통합 Admin API (`POST /api/v1/admin/changes`)를 호출한다. Console은 Git/kubeseal/kubectl을 직접 조작하지 않는다 (**Console Creates, Server Manages** 원칙).
...
`POST /api/v1/admin/changes`에 `secrets` 필드가 포함되면, Config Server는 설정 변경과 함께 시크릿 암호화 + SealedSecret apply를 **단일 atomic commit**으로 처리한다.

```
POST /api/v1/admin/changes 수신 (secrets 필드 포함)
      │
...
Config Agent는 replica=2로 배포되며, **K8s Lease 기반 leader election**으로 하나만 active하게 동작한다 (이중 적용 방지). Leader 장애 시 standby replica가 자동 인계한다. Active leader만 Config Server를 주기적으로 폴링하여 변경을 감지하고, K8s 리소스를 업데이트한다.
...
> Console은 `DELETE /api/v1/admin/changes`로 설정 삭제 후, `POST /api/v1/admin/app-registry/webhook` (action: delete)로 App 등록 해제를 별도 호출한다. 두 호출은 Console의 프로비저닝 파이프라인(Console FR-7)에서 순차 실행된다.
...
| **FR-15** | 헬스체크 / 운영 API | `server` | `/healthz`, `/readyz`, `/api/v1/status` | — | — |
