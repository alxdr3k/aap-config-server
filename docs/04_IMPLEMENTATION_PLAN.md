...
| `P1-M3` | Console integration extensions and production hardening. |  | `in_progress` | `AC-040`~`AC-042` | `docs/01_PRD.md`, `DEC-003` | `EXT-1A.1`~`EXT-1C.4` landed; `EXT-1D.1` is ready next. |
...
| `EXT` | Watch, history, revert, inheritance, batch, webhook, metrics, and HTTP response extensions. | `EXT-1D` | `in_progress` | `EXT-1A` watch slices, `EXT-1B.1`~`EXT-1B.5` history/versioned-read/revert slices, and `EXT-1C.1`~`EXT-1C.4` inheritance slices landed. `EXT-1D.1` is ready next. |
...
| `EXT-1D.1` | `P1-M3` | `EXT` | `EXT-1D` | Add ETag and `If-None-Match` support for non-secret config/env responses. | `CORE-1A.4`, `SECRET-1A.7` | `AC-041` | `defined` | `ready` | `docs/01_PRD.md`, `EXT-1C.4` landed | Start here next. |
