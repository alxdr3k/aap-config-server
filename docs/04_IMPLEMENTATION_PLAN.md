...
| `P1-M3` | Console integration extensions and production hardening. |  | `in_progress` | `AC-040`~`AC-042` | `docs/01_PRD.md`, `DEC-003` | `EXT-1A.1`~`EXT-1D.4` landed; `EXT-1D.5` is ready next. |
...
| `EXT` | Watch, history, revert, inheritance, batch, webhook, metrics, and HTTP response extensions. | `EXT-1D` | `in_progress` | `EXT-1A` watch slices, `EXT-1B.1`~`EXT-1B.5` history/versioned-read/revert slices, `EXT-1C.1`~`EXT-1C.4` inheritance slices, and `EXT-1D.1`~`EXT-1D.4` HTTP response/batch/metrics extensions landed. `EXT-1D.5` is ready next. |
...
| `EXT-1D.5` | `P1-M3` | `EXT` | `EXT-1D` | Add authenticated Git webhook trigger for immediate refresh after config repo changes. | `OPS-1A.1`, `OPS-1A.2` | `AC-041` | `defined` | `ready` | `docs/01_PRD.md`, `EXT-1D.4` landed | Start here next. |
...
- Planned: AAP Console API, Kubernetes API, Bitnami SealedSecrets,
  Config Agent runtime, and authenticated Git webhook refresh.
