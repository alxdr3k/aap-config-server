...
| `P1-M3` | Console integration extensions and production hardening. |  | `in_progress` | `AC-040`~`AC-042` | `docs/01_PRD.md`, `DEC-003` | `EXT-1A.1`~`EXT-1B.2` landed; `EXT-1B.3` is ready next. |
...
| `EXT` | Watch, history, revert, inheritance, batch, webhook, metrics, and HTTP response extensions. | `EXT-1B` | `in_progress` | `EXT-1A` watch slices and `EXT-1B.1`~`EXT-1B.2` history foundation/API landed. `EXT-1B.3` is ready next. |
...
| `EXT-1B.3` | `P1-M3` | `EXT` | `EXT-1B` | Add versioned config/env reads from historical Git commits. | `EXT-1B.1`, `CORE-1A.4` | `AC-040` | `defined` | `ready` | `docs/01_PRD.md`, `EXT-1B.1` landed, `CORE-1A.4` accepted | Start here next. |
