...
| `SECRET` | Secret write/resolve and SealedSecret integration. | `SECRET-1A` | `in_progress` | Runtime boundaries, volume reader, deterministic SealedSecret YAML generation, and K8s apply adapter landed; admin write integration remains planned. |
...
| `SECRET-1A.5` | `P1-M1` | `SECRET` | `SECRET-1A` | Accept `secrets` in admin writes, write metadata plus SealedSecret files in one Git commit, apply to K8s, and reload outcome explicitly. | `SECRET-1A.2`, `SECRET-1A.4`, `CORE-1A.5` | `AC-020` | `defined` | `ready` | ADR-004 | Start here next. |
