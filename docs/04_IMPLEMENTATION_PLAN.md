...
| `P1-M2` | Config Agent rollout path. |  | `in_progress` | `AC-030` | ADR-001, ADR-002 | `AGENT-1A.5` landed; `AGENT-1A.6` is ready next. |
...
| `AGENT` | Config Agent and rollout orchestration. | `AGENT-1A` | `in_progress` | Agent bootstrap, leader election, read polling, rendering, and ConfigMap/Secret apply landed; later slices remain planned until their direct dependencies land. |
...
| `AGENT-1A.6` | `P1-M2` | `AGENT` | `AGENT-1A` | Patch target Deployment annotations to trigger controlled rolling restarts. | `AGENT-1A.5` | `AC-030` | `defined` | `ready` | ADR-002 | Start here next. |
