...
| `P1-M2` | Config Agent rollout path. |  | `in_progress` | `AC-030` | ADR-001, ADR-002 | `AGENT-1A.4` landed; `AGENT-1A.5` is ready next. |
...
| `AGENT` | Config Agent and rollout orchestration. | `AGENT-1A` | `in_progress` | Agent bootstrap, leader election, read polling, and rendering landed; later slices remain planned until their direct dependencies land. |
...
| `AGENT-1A.5` | `P1-M2` | `AGENT` | `AGENT-1A` | Apply target ConfigMap and Secret resources with create/update/patch behavior constrained to configured resource names. | `AGENT-1A.4` | `AC-030` | `defined` | `ready` | ADR-002 | Start here next. |
