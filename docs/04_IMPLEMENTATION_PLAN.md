...
| `P1-M3` | Console integration extensions and production hardening. |  | `in_progress` | `AC-040`~`AC-042` | `docs/01_PRD.md`, `DEC-003` | `EXT-1A.1`~`EXT-1C.2` landed; `EXT-1C.3` is ready next. |
...
| `EXT` | Watch, history, revert, inheritance, batch, webhook, metrics, and HTTP response extensions. | `EXT-1C` | `in_progress` | `EXT-1A` watch slices, `EXT-1B.1`~`EXT-1B.5` history/versioned-read/revert slices, `EXT-1C.1` defaults source parsing, and `EXT-1C.2` internal merge semantics landed. `EXT-1C.3` is ready next. |
...
| `EXT-1C.3` | `P1-M3` | `EXT` | `EXT-1C` | Apply `inherit=true/false` query semantics to config and env var read paths. | `EXT-1C.2`, `CORE-1A.4` | `AC-040` | `defined` | `ready` | `docs/01_PRD.md`, `EXT-1C.2` landed | Start here next. |
