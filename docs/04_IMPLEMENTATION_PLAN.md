...
| `P1-M3` | Console integration extensions and production hardening. |  | `in_progress` | `AC-040`~`AC-042` | `docs/01_PRD.md`, `DEC-003` | `EXT-1A.1`~`EXT-1B.4` landed; `EXT-1B.5` is ready next. |
...
| `EXT` | Watch, history, revert, inheritance, batch, webhook, metrics, and HTTP response extensions. | `EXT-1B` | `in_progress` | `EXT-1A` watch slices and `EXT-1B.1`~`EXT-1B.4` history/versioned-read/revert-plan slices landed. `EXT-1B.5` is ready next. |
...
| `EXT-1B.5` | `P1-M3` | `EXT` | `EXT-1B` | Implement revert commit/push/reload flow, including SealedSecret rollback apply when secret files are restored. | `EXT-1B.4`, `SECRET-1A.4` | `AC-040` | `defined` | `ready` | `docs/01_PRD.md`, `EXT-1B.4` landed, `SECRET-1A.4` landed | Start here next. |
