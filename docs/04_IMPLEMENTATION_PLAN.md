...
| `P1-M2` | Config Agent rollout path. |  | `in_progress` | `AC-030` | ADR-001, ADR-002 | `AGENT-1A.6` landed; `AGENT-1A.7` is ready next. |
...
| `AGENT` | Config Agent and rollout orchestration. | `AGENT-1A` | `in_progress` | Agent bootstrap, leader election, read polling, rendering, ConfigMap/Secret apply, and rollout patch landed; later slices remain planned until their direct dependencies land. |
...
| `AGENT-1A.7` | `P1-M2` | `AGENT` | `AGENT-1A` | Implement leading-edge debounce with cooldown, quiet period, and max-wait controls. | `AGENT-1A.6` | `AC-030` | `defined` | `ready` | ADR-001 | Start here next. |
