...
| `P1-M3` | Console integration extensions and production hardening. |  | `in_progress` | `AC-040`~`AC-042` | `docs/01_PRD.md`, `DEC-003` | `EXT-1A.1` is ready next. |
...
| `EXT` | Watch, history, revert, inheritance, batch, webhook, metrics, and HTTP response extensions. | `EXT-1A` | `in_progress` | `EXT-1A.1` is ready; later EXT slices remain planned until their direct dependencies land. |
...
| `EXT-1A.1` | `P1-M3` | `EXT` | `EXT-1A` | Add store notification and version-wait primitive for long-poll watch endpoints. | `CORE-1A.3` | `AC-040` | `defined` | `ready` | `docs/01_PRD.md`, `CORE-1A.3` accepted | Start here next. |
