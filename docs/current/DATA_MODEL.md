...
| Config Agent fetch state | handled, changed | The fetch loop tracks the last successfully handled config/env versions and treats the first snapshot or any version/updated-at change as changed. Failed handlers do not advance the handled state. |
...
- Config Agent K8s apply/rollout data models are target design only; bootstrap
  Config Server response DTOs, leader election config, and fetch loop state
