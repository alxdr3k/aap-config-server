...
| Store version wait primitive | In-memory notification channel used by config watch and future watch endpoints to wait until the loaded Git version changes. | `internal/store/store.go` |
...
| Store version waiter | waiting, changed, canceled | Waiters return immediately for stale caller versions, wake after successful snapshot version changes, and do not wake on failed reloads that keep serving last-known-good data. The config watch handler maps unchanged timeout to `304 Not Modified`. |
