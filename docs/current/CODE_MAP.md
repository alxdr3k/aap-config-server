...
| Path | Purpose |
|---|---|
| `internal/store/` | In-memory service snapshot, Git-backed reload, resource-scoped version tokens, historical config/env file reads, revert restore planning/application, config/env/secret apply operations, delete operations, history filtering, degraded status. |
...
| `internal/store/*_test.go` | Snapshot reload, resource-scoped versions, version-change waiting, historical config/env reads, revert restore planning/application, history filtering, config/env/secret apply, secret audit logging, delete, degraded behavior, concurrency. |
