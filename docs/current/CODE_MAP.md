...
| `internal/handler/*_test.go` | HTTP routes, config/env watch behavior, versioned and inherited read behavior, history API behavior, revert endpoint behavior, auth, admin write response shape, App Registry webhook auth/cache updates, App Registry status reporting, secret write input cleanup, resolved env var secret reads, secret audit logging, reload/degraded status. |
