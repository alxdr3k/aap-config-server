...
| `internal/handler/` | HTTP routing, request decoding, API key auth, JSON response/error envelope, and config watch long polling. |
...
| `internal/store/*_test.go` | Snapshot reload, version-change waiting, config/env/secret apply, secret audit logging, delete, degraded behavior, concurrency. |
| `internal/gitops/*_test.go` | Local Git clone/pull/commit/delete/snapshot behavior. |
| `internal/handler/*_test.go` | HTTP routes, config watch behavior, auth, admin write response shape, App Registry webhook auth/cache updates, App Registry status reporting, secret write input cleanup, resolved env var secret reads, secret audit logging, reload/degraded status. |
