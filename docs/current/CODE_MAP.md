...
| `internal/handler/` | HTTP routing, request decoding, API key auth, JSON response/error envelope, config/env watch long polling, and history API responses. |
...
| Path | Purpose |
|---|---|
| `internal/store/` | In-memory service snapshot, Git-backed reload, resource-scoped version tokens, config/env/secret apply operations, delete operations, history filtering, degraded status. |
...
| `internal/store/*_test.go` | Snapshot reload, resource-scoped versions, version-change waiting, history filtering, config/env/secret apply, secret audit logging, delete, degraded behavior, concurrency. |
...
| `internal/handler/*_test.go` | HTTP routes, config/env watch behavior, history API behavior, auth, admin write response shape, App Registry webhook auth/cache updates, App Registry status reporting, secret write input cleanup, resolved env var secret reads, secret audit logging, reload/degraded status. |
