...
| `internal/handler/` | HTTP routing, request decoding, API key auth, JSON response/error envelope, and config/env watch long polling. |
...
| Path | Purpose |
|---|---|
| `internal/store/` | In-memory service snapshot, Git-backed reload, resource-scoped version tokens, config/env/secret apply operations, delete operations, degraded status. |
