...
| `TEST-040` | Console extension tests | `internal/store/store_test.go`, `internal/handler/handler_test.go`, `internal/gitops/repo_test.go`; future inheritance tests | `AC-040` store version wait primitive, resource-scoped config/env watch endpoints, git history iterator/classifier, public history API, versioned config/env reads, revert target validation/restore plan, and public revert endpoint flow landed; inheritance tests remain future |
