...
| `TEST-040` | Console extension tests | `internal/store/store_test.go`, `internal/handler/handler_test.go`, `internal/gitops/repo_test.go`; future inheritance admin-write regression tests | `AC-040` store version wait primitive, resource-scoped config/env watch endpoints, git history iterator/classifier, public history API, versioned config/env reads, revert target validation/restore plan, public revert endpoint flow, defaults source parsing, internal merge semantics, and public `inherit` query behavior landed; admin write preservation regression tests remain future |
