...
| `TEST-040` | Console extension tests | `internal/store/store_test.go`, `internal/handler/handler_test.go`, `internal/gitops/repo_test.go`; future public history/revert/inheritance tests | `AC-040` store version wait primitive, resource-scoped config/env watch endpoints, and git history iterator/classifier landed; public history/revert/inheritance tests remain future |
