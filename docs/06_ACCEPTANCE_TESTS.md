...
| `TEST-040` | Console extension tests | `internal/store/store_test.go`, `internal/handler/handler_test.go`, `internal/gitops/repo_test.go`; future inheritance query tests | `AC-040` store version wait primitive, resource-scoped config/env watch endpoints, git history iterator/classifier, public history API, versioned config/env reads, revert target validation/restore plan, public revert endpoint flow, defaults source parsing, and internal merge semantics landed; public inheritance query tests remain future |
