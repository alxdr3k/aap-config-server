...
| `TEST-040` | Console extension tests | `internal/store/store_test.go`, `internal/handler/handler_test.go`; future history/revert/inheritance tests | `AC-040` store version wait primitive plus resource-scoped config/env watch endpoints landed; history/revert/inheritance tests remain future |
