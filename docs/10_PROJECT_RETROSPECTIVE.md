...
- Existing `FR-*` IDs differ from boilerplate `REQ-*` examples; `DEC-002`
  keeps `FR-*` canonical.
- ADR-003 describes finer-grained concurrency than current code implements;
  `ADR-005` accepts the current global serialization boundary for Phase-1.
...
No open P0-M3 retrospective actions.
