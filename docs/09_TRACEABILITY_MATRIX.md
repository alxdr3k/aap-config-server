...
| `TRACE-008` |  | `ADR-001`, `ADR-002`, `DEC-003` | `FR-9` | `AC-030` / `TEST-030` | `P1-M2` | `AGENT` | `AGENT-1A` | `AGENT-1A.1`~`AGENT-1A.8` | Config Agent bootstrap, leader election, read polling, rendering, ConfigMap/Secret apply, rollout patch, debounce, image build target, RBAC/deployment handoff examples, and fake-client e2e smoke coverage landed. |
...
| `TRACE-013` |  |  | `FR-6` | `AC-040` / `TEST-040` | `P1-M3` | `EXT` | `EXT-1A` | `EXT-1A.1`~`EXT-1A.3` | Store version-wait primitive plus resource-scoped config/env watch endpoints landed. |
| `TRACE-014` |  |  | `FR-13`, `FR-14` | `AC-040` / `TEST-040` | `P1-M3` | `EXT` | `EXT-1B` | `EXT-1B.1`~`EXT-1B.5` | `EXT-1B.1` git history iterator/classifier, `EXT-1B.2` public history API, `EXT-1B.3` versioned config/env reads, `EXT-1B.4` revert target validation/restore plan, and `EXT-1B.5` public revert endpoint/commit flow landed. |
| `TRACE-015` |  |  | `FR-10` | `AC-040` / `TEST-040` | `P1-M3` | `EXT` | `EXT-1C` | `EXT-1C.1`~`EXT-1C.4` | `EXT-1C.1` defaults source parsing, `EXT-1C.2` internal merge semantics, `EXT-1C.3` public `inherit` query behavior, and `EXT-1C.4` admin write preservation regression landed. |
| `TRACE-016` |  |  | Operational extensions | `AC-041` / `TEST-041` | `P1-M3` | `EXT` | `EXT-1D` | `EXT-1D.1`~`EXT-1D.5` | `EXT-1D.1` ETag/`If-None-Match`, `EXT-1D.2` gzip, `EXT-1D.3` batch read support, `EXT-1D.4` Prometheus metrics, and `EXT-1D.5` authenticated Git webhook refresh landed. |
| `TRACE-017` | `Q-003` | `DEC-003` | Production hardening | `AC-042` / `TEST-042` | `P1-M3` | `HARDEN` | `HARDEN-1A` | `HARDEN-1A.1`~`HARDEN-1A.5` | `HARDEN-1A.1` parser-level schema validation landed for config, env vars, defaults, and secret metadata files; rate, integration/load, and deployment handoff work remains planned. |
...
| `TRACE-028` |  |  | `FR-8`, `FR-15` | `AC-021` / `internal/handler/handler_test.go`, `internal/registry/*_test.go` | `P1-M1` | `REGISTRY` | `REGISTRY-1A` | `REGISTRY-1A.3` | `/api/v1/status` now reports App Registry load/cache state and degraded components while `/readyz` remains non-blocking for registry-only Console outages. |
