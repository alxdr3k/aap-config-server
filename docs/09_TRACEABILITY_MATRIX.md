...
| `TRACE-014` |  |  | `FR-13`, `FR-14` | `AC-040` / `TEST-040` | `P1-M3` | `EXT` | `EXT-1B` | `EXT-1B.1`~`EXT-1B.5` | `EXT-1B.1` git history iterator/classifier, `EXT-1B.2` public history API, `EXT-1B.3` versioned config/env reads, and `EXT-1B.4` revert target validation/restore plan landed; public revert endpoint/commit flow remains leaf-planned. |
