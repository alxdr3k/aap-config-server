...
| `TRACE-016` |  |  | Operational extensions | `AC-041` / `TEST-041` | `P1-M3` | `EXT` | `EXT-1D` | `EXT-1D.1`~`EXT-1D.5` | `EXT-1D.1` ETag/`If-None-Match` and `EXT-1D.2` gzip support landed for non-secret config/env reads; batch, metrics, and Git webhook work remain planned. |
