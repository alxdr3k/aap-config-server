...
| `TRACE-016` |  |  | Operational extensions | `AC-041` / `TEST-041` | `P1-M3` | `EXT` | `EXT-1D` | `EXT-1D.1`~`EXT-1D.5` | `EXT-1D.1` ETag/`If-None-Match`, `EXT-1D.2` gzip, and `EXT-1D.3` batch read support landed for non-secret config/env reads; metrics and Git webhook work remain planned. |
